Rollbar.configure do |config|
  ...
  config.access_token = Rails.application.credentials.dig(:rollbar, :token)

  # Here we'll disable in 'test':
  if Rails.env.test? || config.access_token.blank?
    config.enabled = false
  end

  if File.file?("../../revisions.log")
    deploy = File.open("../../revisions.log").read.split("\n").last.split(" ")
...
Rollbar::UserInformer.user_information_full = <<~HTML
  <p><a href="#{Rollbar.notifier.configuration.web_base}/instance/uuid?uuid={{error_uuid}}" target="_blank">
