...
RSpec.describe SitemapGeneratorCommand, type: :command do
  ...
  describe '#process' do
    context 'when generating sitemap successfully' do
      ...
      it 'creates sitemap files' do
        ...
        expect(File.exist?(Rails.public_path.join('sitemaps', 'sitemap.xml.gz'))).to be_truthy
      end
  ...
  describe '#add_blog_posts_urls' do
    let(:sitemap) { double('sitemap') }
    let!(:post1) { create(:post, title: "First Post") }
    let!(:post2) { create(:post, title: "Second Post") }

    it 'adds all blog posts with correct settings' do
      expect(sitemap).to receive(:add).with(post_path(post1), 
                                            lastmod: post1.updated_at,
                                            priority: 0.8,
                                            changefreq: "weekly")
      expect(sitemap).to receive(:add).with(post_path(post2),
                                            lastmod: post2.updated_at,
                                            priority: 0.8,
                                            changefreq: "weekly")
